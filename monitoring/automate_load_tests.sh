...
if [ "$SKIP_TESTS" = false ]; then
    ...
    echo ""
    log_message "🔥 Lancement du test (${MIN} minute)"
    if ! k6 run --vus "$VUS" --duration "${MIN}m" monitoring/load_test.js; then
...
echo "📋 Résumé:"
if [ "$SKIP_TESTS" = true ]; then
    echo "   • Mode: Rapport des 30 dernières minutes (tests non exécutés)"
else
    echo "   • Tests effectués: 1 (1min)"
    echo "   • Utilisateurs virtuels: $VUS"
    echo "   • Durée totale: ~1 minute"
fi
